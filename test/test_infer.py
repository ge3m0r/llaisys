...
if __name__ == "__main__":
    ...
    if args.test:
        print(f"\nHF tokens length: {len(tokens)}")
        print(f"llaisys tokens length: {len(llaisys_tokens)}")
        print(f"\nHF tokens: {tokens[:20]}...")
        print(f"llaisys tokens: {llaisys_tokens[:20]}...")

        # Find first difference
        for i, (h, l) in enumerate(zip(tokens, llaisys_tokens)):
            if h != l:
        ...
        assert llaisys_tokens == tokens, f"Tokens don't match!"
